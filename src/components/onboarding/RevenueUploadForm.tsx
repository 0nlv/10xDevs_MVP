/**
 * RevenueUploadForm Component
 * 
 * React island for revenue file selection, upload submission, and preview display.
 * Manages upload state (idle → uploading → preview → error).
 */
...
export default function RevenueUploadForm() {
  ...
  const handleContinue = () => {
    if (uploadId) {
      window.location.href = `/onboarding/step-2?upload_id=${uploadId}`;
    }
  };
      ...
      {state === 'preview' && preview && (
        <div className="space-y-6">
          ...
          <CSVPreviewTable
            ...
            rows={preview.rows}
            caption="Preview of your revenue data (first 5 rows)"
          />
